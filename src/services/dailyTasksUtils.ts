...
import { MembershipLevel, PROFIT_RATES, DAILY_TASKS_COUNT } from './dailyTasks';
...
export const getAvailableTasks = async (userId: string) => {
  try {
    ...
    // استخدام القيم المحددة في DAILY_TASKS_COUNT
    const maxTasks = DAILY_TASKS_COUNT[membershipLevel] || 3;
...
export const completeTaskWithReward = async (userId: string, taskId: string): Promise<{ success: boolean; reward?: number; error?: string }> => {
  try {
    ...
    const taskProfitRate = {
      min: profitRate.min / taskCount,
      max: profitRate.max / taskCount
    };
    // اختيار معدل ربح عشوائي بين الحد الأدنى والحد الأقصى للمهمة الواحدة
    const randomProfitRate = taskProfitRate.min + (Math.random() * (taskProfitRate.max - taskProfitRate.min));
    const rewardAmount = userBalance * (randomProfitRate / 100);
...
export const getUserDailyTasksData = async (userId: string) => {
  try {
    ...
    // تحديد عدد المهام المتاحة بناءً على مستوى العضوية
    const totalTasks = DAILY_TASKS_COUNT[membershipLevel] || 3;
